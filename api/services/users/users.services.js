...
class Users {
    ...
    async syncUsersDB() {
        var usersRequest = await database.query("SELECT uId AS 'userId', email, username, createdOn FROM users");
    ...
    async create(data, params) {
        ...
        this.users.forEach(user => {
            if (user.userId > newUserId) { newUserId = user.userId }
        });
        const creationTime = Date.getTime();
        const sql = "INSERT INTO users (uId, email, username, password, createdOn " +
            "VALUES ('" + (newUserId + 1) + "', '" + data.email + "', '" + data.username + "', '" + data.password + "', '" + creationTime + "')";
        database.query(sql)
        const newUser = {
            userId: (newUserId + 1),
            email: data.email,
