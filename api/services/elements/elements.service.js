...
class Elements {
  async getAllByTopics() {
    var sortedElements = [];
    var elementsQuery = await database.query(
      "SELECT eId AS 'key', label, sortIndex AS 'index', state, tId AS 'topicKey' FROM elements"
    );

    elementsQuery.forEach((element) => {
      var sortedElementIx = sortedElements.findIndex(
        (e) => e.topicKey === element.topicKey
      );
  ...
  async getByTopic(topicId) {
    var elementsQuery = await database.query(
      "SELECT eId AS 'key', label, state, sortIndex AS 'index' FROM elements WHERE tId = '" +
        topicId +
        "'"
    );
  ...
  async find(params) {
    if (params.query.topic === undefined) {
      var elementsQuery = await database.query(
        "SELECT eId AS 'key', tId AS 'topicKey', label, state, sortIndex AS 'index' FROM elements"
      );
      elementsQuery.map((e) => {
        e.state = Boolean(e.state);
        return e;
  ...
  async get(id, params) {
    var elementsQuery = await database.query(
      "SELECT eId AS 'key', tId AS 'topicKey', label, state, sortIndex AS 'index' FROM elements WHERE eId = '" +
        id +
        "'"
    );
  ...
  async create(data, params) {
    ...
    var keyFinder = await database.query(
      "SELECT MAX(eId) AS 'maxEId' FROM elements"
    );
    var indexFinder = await database.query(
      "SELECT MAX(sortIndex) AS 'maxSortIndex' FROM elements WHERE tId = '" +
        data.topicKey +
        "'"
    );
    ...
    await database.query(
      "INSERT INTO elements (eId, sortIndex, label, state, tId) VALUES ('" +
        newKey +
        "', '" +
        newSortIndex +
        "', '" +
  ...
  async patch(id, data, params) {
    ...
    if (
      params.query.switchState !== undefined &&
      params.query.switchState !== 0
    ) {
      const newState = Number(!result.state);
      database.query(
        "UPDATE elements SET state = '" +
          newState +
          "' WHERE elements.eId = '" +
          id +
          "'"
