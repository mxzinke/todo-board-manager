...
class Topics {
  ...
  async find(params) {
    ...
    if (params.query.withoutTitle === undefined) {
      topicQuery = await database.query(
        "SELECT tId AS 'key', title, sortIndex AS 'index' FROM topics"
      );
    } else {
      topicQuery = await database.query(
        "SELECT tId AS 'key', sortIndex AS 'index' FROM topics"
      );
    }
    ...
    if (params.query.withoutElements === undefined) {
      ...
      allTopics.topics.forEach((row, index) => {
        var relevantIx = sortedElements.findIndex(
          (e) => e.topicKey === row.key
        );

        var relevantEle =
          relevantIx !== -1 ? sortedElements[relevantIx].elements : [];
  ...
  async get(id, params) {
    var topicQuery = await database.query(
      "SELECT tId AS 'key', title, sortIndex AS 'index' FROM topics WHERE tId = '" +
        id +
        "'"
    );
  ...
  async create(data, params) {
    ...
    var indexFinder = await database.query(
      "SELECT MAX(sortIndex) AS 'maxSortIndex', MAX(tId) AS 'maxTId' FROM topics"
    );
    ...
    await database.query(
      "INSERT INTO topics (tId, sortIndex, title) VALUES ('" +
        newKey +
        "', '" +
        newSortIndex +
        "', '" +
